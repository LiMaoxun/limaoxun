...
class GetTFPose
{
  public:
    ...
    void GetPose(std::string source_frame, std::string target_frame, double *out_x, double *out_y, double *out_z, double *out_roll, double *out_pitch, double *out_yaw, bool *out_flag);
...
void GetTFPose::GetPose(std::string source_frame, std::string target_frame, double *out_x, double *out_y, double *out_z, double *out_roll, double *out_pitch, double *out_yaw, bool *out_flag)
{
    //Get Pose from TF listener
    //printf("first_delay_flag=%d, rev_delay_cnt=%d rev_delay=%d\n", first_delay_flag, rev_delay_cnt, rev_delay);
    if (first_delay_flag)
    {
        ...
        try{
            ...
            y=rev_tf.getOrigin().y();
            z=rev_tf.getOrigin().z();
            tf_pose=Quat_to_AllEuler_TF(rev_tf.getRotation());
            roll=tf_pose.roll;
            pitch=tf_pose.pitch;
            yaw=tf_pose.yaw;
            now_time=ros::Time::now().toSec();
